...
def build_dashboard() -> None:
    ...
    with st.sidebar:
        ...
        # Кнопка завантаження демонстраційного CSV-файлу
        demo_bytes = generate_demo_csv()
        st.download_button(
            label="⬇️ Завантажити тестовий CSV",
            data=demo_bytes,
            file_name="demo_network_logs.csv",
            mime="text/csv",
            help="Натисніть, щоб отримати готовий тестовий файл",
        )

        st.markdown("---")

        # Віджет завантаження файлу від користувача
        uploaded_file = st.file_uploader(
            "Оберіть CSV-файл з логами",
            type=["csv"],
            help="Обов'язкові колонки: Source IP, Destination IP, "
                 "Protocol, Port, Size, Time",
        )

    # ---- Якщо файл не завантажено — показуємо заглушку ----
    if uploaded_file is None:
        st.info(
            "👆 Завантажте CSV-файл з логами мережевої активності або "
            "спочатку скачайте тестовий файл у бічній панелі.",
            icon="ℹ️",
        )
        # Показуємо очікувану структуру файлу
        st.markdown('<div class="section-header">Очікувана структура CSV-файлу</div>',
                    unsafe_allow_html=True)
        example_df = pd.DataFrame({
            "Source IP":      ["192.168.1.10", "10.0.0.99"],
            "Destination IP": ["8.8.8.8",      "192.168.1.5"],
            "Protocol":       ["TCP",           "UDP"],
            "Port":           [443,             53],
            "Size":           [1500,            256],
            "Time":           ["2024-06-01 10:00:00", "2024-06-01 10:00:01"],
        })
        st.dataframe(example_df, use_container_width=True)
        return   # Зупиняємо виконання — далі нема що показувати
